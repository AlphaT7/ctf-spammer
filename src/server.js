...
io.onConnection((channel) => {
  ...
  channel.on('new-game', () => {
    const id = 'GAME-' + Math.random().toString(36).substring(2, 6).toUpperCase()
    const game = {
      id,
...
app.get('/style.css', async (_req, reply) => {
  const css = fs.readFileSync(path.join(staticDir, 'css', 'style.css'), 'utf-8')
  reply.type('text/css').send(css)
})

app.get('/main.js', async (_req, reply) => {
  const js = fs.readFileSync(path.join(staticDir, 'js', 'main.js'), 'utf-8')
...
app.get('/game.js', async (_req, reply) => {
  const js = fs.readFileSync(path.join(staticDir, 'js', 'game.js'), 'utf-8')
  reply.type('application/javascript').send(js)
})
